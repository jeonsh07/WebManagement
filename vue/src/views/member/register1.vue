          ...
          <v-layout wrap class="pt-3">
            ...
            <v-flex xs12 md4>
              <Vuetify-filepond
                ...
                @deleteCallback="componentCallback_deletefileCallback"
              />
            </v-flex>
            <v-flex xs12 md12>
  ...
  methods: {
    ...
    doQuery(memberCode) {
      ...
      this.$BaseCommon.restful
        .call("/member/GetList", { member_code: memberCode })
        .then((res) => {
          if (res.length === 1) {
    ...
    componentCallback_uploadCallback: function(name, response) {
      console.log(name)
      console.log(response)

      if (response.length > 0) {
        this.formData.member_img = response[0].oriFileName
        this.formData.member_imgurl = response[0].attachmentUrl
        this.formData.member_imgsize = response[0].fileSize
      }
    },
    componentCallback_deletefileCallback: function(res) {
      console.log(res)
      if (this.formData.member_code > 0) {
        console.log(this.formData.member_code)
      }
    },
  },
  mounted() {
